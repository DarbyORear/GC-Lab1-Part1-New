...
public class GC_Lab1_Part1 {
	...
	numTwoLength = String.valueOf(userNumTwo).length();
	 ...
	 /*If the second number is not the same length as the first, ask user to input
	 * another number. Repeat this step until second number is the same length as
	 * the first.*/
	...
	if (numTwoLength == numOneLength) {
		...
		sumMods = modTwo + modOne;
		 ...
		 /*Divide each number by 10. userNumOne and userNumTwo will take the decimal
		 * (not floating) value of each dividend. This will basically have the effect of
	...
	//When the loop iterates again, the second to last digit of each number will be separated out and evaluated.
